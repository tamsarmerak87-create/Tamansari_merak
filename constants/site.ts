import type { AgendaItem, GalleryItem, NewsItem, PublicService, Statistic } from "@/types";
...
export const services: PublicService[] = [
    "Domisili,KK,KTP,Kelahiran,Kematian,Nikah,Usaha,Pindah,Datang,SKTM".split(",").map((name) => ({
        id: name.toLowerCase(), title: `Surat ${name}`, category: "administrasi" as const,
        description: `Pengajuan dan pelacakan dokumen ${name} secara digital.`,
        requirements: ["KTP/KK", "Formulir pengajuan", "Dokumen pendukung"], duration: "1 hari kerja", online: true,
    })),
    [{ id: "pengaduan", title: "Pengaduan Masyarakat", category: "pengaduan" as const, description: "Kirim laporan dengan foto, lokasi, QR tiket, dan tracking progres.", requirements: ["Identitas pelapor", "Deskripsi", "Foto/lokasi opsional"], duration: "SLA 3x24 jam", online: true }],
    [{ id: "posbankum", title: "POSBANKUM", category: "posbankum" as const, description: "Booking konsultasi hukum, nomor antrian, dan tracking hasil konsultasi.", requirements: ["Identitas", "Topik konsultasi"], duration: "Sesuai jadwal", online: true }],
].flat();
