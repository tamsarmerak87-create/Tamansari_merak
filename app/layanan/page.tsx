...
export default function LayananPage() {
    return (
        <main className="mesh-bg min-h-screen overflow-hidden">
                        ...
                        return (
                            <MotionShell key={service.id} delay={index * 0.025}>
                                <GlassCard className="group flex h-full flex-col rounded-[2rem] p-6 transition hover:-translate-y-2 hover:bg-white/85">
                                    ...
                                    <div className="mt-5 rounded-3xl border border-white/70 bg-white/55 p-4"><p className="flex items-center gap-2 text-sm font-black text-gov-900"><Clock3 size={16} /> Estimasi {service.duration}</p><p className="mt-3 text-xs font-black uppercase tracking-[0.2em] text-slate-650">Syarat utama</p><ul className="mt-2 space-y-1 text-sm font-semibold text-slate-650">{service.requirements.map((item) => <li key={item}>- {item}</li>)}</ul></div>
            ...
            <Section className="pb-24 pt-0"><GlassCard className="grid gap-6 rounded-[2.5rem] p-8 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-xs font-black uppercase tracking-[0.28em] text-accent-700">Standar pelayanan</p><h2 className="mt-3 font-display text-4xl font-black text-gov-950">Transparan dari syarat sampai estimasi.</h2><p className="mt-3 leading-8 text-slate-650">Portal menampilkan informasi resmi dan mengarahkan warga ke kanal pengajuan sesuai jenis layanan.</p></div><div className="grid size-24 place-items-center rounded-[2rem] bg-gradient-to-br from-gov-950 to-accent-600 text-white shadow-glass"><ShieldCheck size={42} /></div></GlassCard></Section>
        </main>
    );
}