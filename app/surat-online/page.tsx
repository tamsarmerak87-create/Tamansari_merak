...
export default function SuratOnlinePage() {
    ...
    const [form, setForm] = useState<FormState>({ ...emptyForm, serviceId: selectedId });
    ...
    const selectedService = useMemo(() => serviceCatalog.find((item) => item.id === selectedId) ?? serviceCatalog[0], [selectedId]);
    ...
    function validate() {
        ...
        if (form.nik && !/^\d{16}$/.test(form.nik)) next.nik = "NIK harus 16 angka";
        if (form.kk && !/^\d{16}$/.test(form.kk)) next.kk = "Nomor KK harus 16 angka";
        if (form.phone && !/^(\+62|62|0)8[1-9][0-9]{6,11}$/.test(form.phone)) next.phone = "Nomor HP tidak valid";
        if (form.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) next.email = "Email tidak valid";
        (["ktp", "kk", "support"] as FileKey[]).forEach((key) => {
    ...
    function submit(e: FormEvent) {
        ...
        window.setTimeout(() => {
            setTicket(`TMS-${new Date().getFullYear()}-${String(Math.floor(100000 + Math.random() * 899999))}`);
...
function ApplicantForm({ form, errors, update, setSelectedId }: { form: FormState; errors: Record<string, string>; update: (name: keyof FormState, value: string | boolean) => void; setSelectedId: (id: string) => void }) {
...
function Success({ ticket, service, estimate }: { ticket: string; service: string; estimate: string }) {
    return <div className="mt-8 rounded-[24px] border border-emerald-200 bg-emerald-50 p-6"><CheckCircle2 className="text-emerald-600" size={40} /><h2 className="mt-4 text-3xl font-black text-gov-950">Permohonan berhasil dikirim</h2><div className="mt-5 grid gap-3 md:grid-cols-2"><p><b>Nomor Tiket:</b> {ticket}</p><p><b>Tanggal:</b> {new Date().toLocaleDateString("id-ID")}</p><p><b>Jenis Pelayanan:</b> {service}</p><p><b>Estimasi selesai:</b> {estimate}</p></div><div className="mt-6 flex flex-wrap gap-3"><div className="grid size-28 place-items-center rounded-3xl bg-white text-gov-950"><QrCode size={76} /></div><Button type="button" variant="primary"><Download size={18} />Download Bukti Pengajuan</Button><Button type="button" variant="glass"><Printer size={18} />Cetak Bukti</Button></div></div>;
}
...
function StatusResult() {
    return <div className="mt-6 rounded-[24px] bg-gov-50 p-5"><p className="font-black text-gov-950">Status: Diproses</p><div className="mt-4 h-3 overflow-hidden rounded-full bg-white"><div className="h-full w-[58%] bg-gradient-to-r from-accent-300 to-emerald-500" /></div><div className="mt-5 grid gap-3">{timeline.map((item, i) => <div key={item} className="flex items-center gap-3"><span className={cn("grid size-8 place-items-center rounded-full", i < 3 ? "bg-emerald-500 text-white" : "bg-white text-slate-400")}><Check size={16} /></span><span className="font-bold">{item}</span></div>)}</div></div>;
}