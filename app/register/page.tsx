...
export default function RegisterPage() {
    ...
    function selectFile(key: DocKey, event: ChangeEvent<HTMLInputElement>) {
        const file = event.target.files?.[0] ?? null;
        event.target.value = "";
        if (!file) return;
        if (!ACCEPTED_TYPES.includes(file.type)) { setToast({ type: "error", message: "Format foto harus JPG, JPEG, PNG, atau WEBP." }); return; }
        if (file.size > MAX_FILE_SIZE) { setToast({ type: "error", message: "Foto terlalu besar. Silakan pilih foto maksimal 2 MB." }); return; }
    ...
    async function submit(event: FormEvent) {
        ...
        try {
            ...
            setStep(3); setToast({ type: "success", message: "Registrasi berhasil. Data menunggu verifikasi petugas." });
            window.setTimeout(() => router.push("/verify"), 1200);
