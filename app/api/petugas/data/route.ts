...
export async function GET(request: NextRequest) {
    ...
    const [activeResult, allStagesResult, submissionsResult, officersResult, auditsResult, wargaResult, petugasNotificationsResult] = await Promise.all([
        ...
        safeRows<VerificationRow>("verifikasi_pengajuan.all", (() => { let q = supabase.from("verifikasi_pengajuan").select("id,pengajuan_id,tahap,nama_tahap,role_petugas,status,petugas_id,catatan,created_at,acted_at"); if (!isLurah && workflowRole !== "kepala_seksi") q = q.or(`role_petugas.eq.${workflowRole},petugas_id.eq.${session.profile.id}`); return q.order("tahap", { ascending: true }); })(), warnings),
    ...
    const kasiSubmissionIds = workflowRole === "kepala_seksi"
        ? Array.from(new Set(allStages.filter((stage) => [1, 2].includes(stage.tahap)).map((stage) => stage.pengajuan_id)))
            .filter((pengajuanId) => {
                const stages = stagesByPengajuan.get(String(pengajuanId)) ?? [];
                const first = stages.find((stage) => Number(stage.tahap) === 1);
                const second = stages.find((stage) => Number(stage.tahap) === 2);
                const submission = submissionMap.get(String(pengajuanId));
                return Boolean(submission && first && second
                    && first.role_petugas === "staff_pelayanan"
                    && second.role_petugas === "petugas_lapangan"
                    && !first.petugas_id && !second.petugas_id
                    && ["Menunggu", "Diproses"].includes(String(first.status))
                    && ["Menunggu", "Diproses"].includes(String(second.status))
                    && !isFinalSubmissionStatus(String(submission.workflow_status ?? submission.status)));
            })
        : [];
    const kasiTasks: AnyRow[] = kasiSubmissionIds
        .map((pengajuanId) => (stagesByPengajuan.get(String(pengajuanId)) ?? []).find((stage) => Number(stage.tahap) === 1))
        .filter((stage) => Boolean(stage))
        .map((stage) => enrichStage(stage as VerificationRow))
    ...
    const visibleSubmissions = isLurah ? (submissionsResult.data ?? []) : (submissionsResultDetail.data ?? []);
    const normalizedStatus = (row: AnyRow) => String(row.status ?? row.workflow_status ?? "").toUpperCase().replace(/\s+/g, "_");
    const wargaProfiles = wargaResult.data ?? [];
    const warga = { menunggu: wargaProfiles.filter((row: AnyRow) => row.status_verifikasi === "Belum Terverifikasi").length, terverifikasi: wargaProfiles.filter((row: AnyRow) => row.status_verifikasi === "Terverifikasi").length, ditolak: wargaProfiles.filter((row: AnyRow) => row.status_verifikasi === "Ditolak").length, total: wargaProfiles.length };
    const service_trend = Array.from({ length: 6 }, (_, offset) => { const d = new Date(); d.setDate(1); d.setMonth(d.getMonth() - (5 - offset)); const y = d.getFullYear(); const m = d.getMonth(); return { label: d.toLocaleDateString("id-ID", { month: "short" }), total: visibleSubmissions.filter((row: AnyRow) => { const created = new Date(String(row.created_at ?? "")); return !Number.isNaN(created.getTime()) && created.getFullYear() === y && created.getMonth() === m; }).length }; });
    const pelayanan = { masuk: workflowRole === "kepala_seksi" ? kasiTasks.length : isLurah ? visibleSubmissions.filter((row: AnyRow) => !isFinalSubmissionStatus(normalizedStatus(row))).length : pengajuanTasks.length, proses: isLurah ? visibleSubmissions.filter((row: AnyRow) => !isFinalSubmissionStatus(normalizedStatus(row))).length : pengajuanTasks.filter((row: AnyRow) => !isFinalSubmissionStatus(normalizedStatus(row))).length, selesai: visibleSubmissions.filter((row: AnyRow) => ["SELESAI", "DISETUJUI"].includes(normalizedStatus(row)) || String(row.status).toLowerCase() === "selesai").length, ditolak: visibleSubmissions.filter((row: AnyRow) => ["DITOLAK", "DIBATALKAN"].includes(normalizedStatus(row)) || String(row.status).toLowerCase() === "ditolak").length };
    const stats = { menunggu: tasks.length, tugas_pengajuan: pengajuanTasks.length, verifikasi_warga: wargaTasks.length, pengajuan_masuk: kasiTasks.length, diproses: history.length, dikembalikan: history.filter((row) => /kembali|revisi|dikembalikan/i.test(`${row.action ?? row.aksi ?? row.status ?? row.status_sesudah ?? ""}`)).length, warga, pelayanan, service_trend, lurah: { total: totalResult.total, staff: stageCounts["1"] ?? 0, lapangan: stageCounts["2"] ?? 0, kasi: stageCounts["3"] ?? 0, seklur: stageCounts["4"] ?? 0, lurah: stageCounts["5"] ?? 0, selesai: totalResult.selesai } };
