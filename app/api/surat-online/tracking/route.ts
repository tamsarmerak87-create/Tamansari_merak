...
async function getMonitorData() {
    ...
    const [{ count: total, error: totalError }, { data, error }] = await Promise.all([
        ...
        client
            ...
            .select("id, nomor_pengajuan, jenis_surat, status, created_at, updated_at, layanan(nama), tracking_pengajuan(status, created_at)")
...
export async function GET(request: Request) {
    try {
        ...
        const [rows, monitor] = await Promise.all([searchSubmission(query) as Promise<TrackingRow[]>, getMonitorData()]);
