...
import { accessWargaDokumen, deleteMyPengajuan, getMyPengajuanDetail, isServiceResultDocument, isWargaUploadedDocument, resubmitMyPengajuan, type DokumenPengajuan, type WargaPengajuan } from "@/services/warga-pengajuan.service";
...
function isIssuedLetter(document: DokumenPengajuan) {
    return isServiceResultDocument(document) && ["TERBIT", "SIGNED", "FINAL", "ISSUED"].includes(String(document.status ?? "").trim().toUpperCase()) && Boolean(document.id && document.pengajuan_id && document.url_file);
}
...
export default function DetailPengajuanPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function accessDocument(documentId: string, download = false) {
        try {
            await accessWargaDokumen(documentId, download, "final-letter");
        } catch (error) {
            toast.error(error instanceof Error ? error.message : "Dokumen belum dapat diakses. Silakan hubungi administrator.");
        }
    }
    ...
    useEffect(() => { if (!user || !profile) { if (!loading) void Promise.resolve().then(() => setFetching(false)); return; } void (async () => { try { setFetching(true); setItem(await getMyPengajuanDetail(id, profile)); } catch (error) { console.error(error); setItem(null); } finally { setFetching(false); } })(); }, [loading, user, profile, id]);
    ...
    const allDocuments = item.dokumen_pengajuan ?? [];
    const documents = allDocuments.filter((doc) => Boolean(doc.url_file) && isWargaUploadedDocument(doc));
    const issuedLetter = allDocuments.find(isIssuedLetter);
    ...
    return <main className="min-h-screen bg-[#F7F8F5] px-4 py-6 text-[#172033] sm:px-8 lg:px-16"><section className="mx-auto max-w-6xl"><BackButton onClick={() => router.push("/dashboard")} className="mb-4" />
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_300px]"><div className="space-y-6"><section className="rounded-[24px] border border-[#E8E8E8] bg-white p-5 shadow-sm sm:p-6"><h2 className="text-xl font-black">Informasi Pengajuan</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Info label="Nomor Pengajuan" value={item.nomor_pengajuan || "-"} /><Info label="Layanan" value={name} /><Info label="Keperluan" value={item.keperluan || "-"} /><Info label="Tanggal Pengajuan" value={formatDate(item.created_at)} /><Info label="Status" value={`${summary?.icon ?? "○"} ${summary?.label ?? "Menunggu proses"}`} /></div></section>
            ...
            <section className="rounded-[24px] border border-[#E8E8E8] bg-white p-5 shadow-sm sm:p-6"><h2 className="text-xl font-black">Dokumen Pengajuan</h2>{documents.length ? <div className="mt-4 divide-y divide-slate-100">{documents.map((doc) => { const label = docLabel(doc.jenis, doc.nama_file); return <div key={doc.id ?? `${item.id}-${doc.url_file}`} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-slate-100"><FileText size={18} /></span><div><p className="font-black">{label}</p><p className="text-xs font-bold text-slate-500">{formatDate(doc.created_at ?? item.created_at)}</p></div></div>{doc.id ? <div className="flex gap-2"><button type="button" className="inline-flex min-h-10 items-center justify-center rounded-xl border border-[#DADDE3] bg-white px-4 text-sm font-black hover:bg-slate-50" onClick={() => void accessDocument(doc.id!, false)}>Lihat</button><button type="button" className="inline-flex min-h-10 items-center justify-center rounded-xl bg-[#FFC400] px-4 text-sm font-black hover:bg-[#FFD84D]" onClick={() => void accessDocument(doc.id!, true)}>Download</button></div> : <p className="text-sm font-bold text-red-700">Dokumen belum dapat diakses. Silakan hubungi administrator.</p>}</div>; })}</div> : <p className="mt-4 font-bold text-slate-500">Belum ada dokumen untuk pengajuan ini.</p>}</section>
            {complete ? <section className="rounded-[24px] border border-emerald-200 bg-emerald-50/60 p-5 shadow-sm sm:p-6"><h2 className="text-xl font-black uppercase tracking-wide text-emerald-900">Surat Hasil Pengajuan</h2>{issuedLetter ? <div className="mt-4 flex flex-col gap-4 rounded-2xl border border-emerald-100 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-emerald-100 text-emerald-800"><FileText size={20} /></span><div><p className="font-black">{name}</p><p className="font-bold text-emerald-700">Surat sudah diterbitkan</p><p className="text-xs font-bold text-slate-500">{formatDate(item.issued_at ?? issuedLetter.created_at ?? item.selesai_at)}</p></div></div><div className="flex flex-wrap gap-2"><button type="button" className="inline-flex min-h-10 items-center justify-center rounded-xl border border-emerald-300 bg-white px-4 text-sm font-black text-emerald-900 hover:bg-emerald-50" onClick={() => void accessDocument(issuedLetter.id!, false)}>LIHAT SURAT</button><button type="button" className="inline-flex min-h-10 items-center justify-center rounded-xl bg-[#FFC400] px-4 text-sm font-black hover:bg-[#FFD84D]" onClick={() => void accessDocument(issuedLetter.id!, true)}>DOWNLOAD PDF</button></div></div> : <p className="mt-4 font-bold text-slate-600">Surat sedang diproses.</p>}</section> : null}</div>
