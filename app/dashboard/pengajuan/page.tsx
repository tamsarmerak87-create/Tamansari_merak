...
import { BarcodePelayanan } from "@/components/pengajuan/barcode-pelayanan";
...
export default function SemuaPengajuanPage() {
    ...
    return <main className="min-h-screen bg-[#F7F8F5] px-4 pb-28 pt-6 text-[#172033] sm:px-8 lg:px-10">
        ...
        </section>{barcodeItem && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true"><div className="w-full max-w-2xl rounded-[28px] bg-white p-4 shadow-2xl"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-black">Barcode Pelayanan</h2><button aria-label="Tutup modal barcode" onClick={() => setBarcodeItem(null)} className="rounded-full px-3 py-1 font-black hover:bg-slate-100">x</button></div><BarcodePelayanan nomorPengajuan={agenda(barcodeItem)} status={barcodeItem.status} tanggal={formatDate(barcodeItem.created_at)} /></div></div>}
    </main>;
}