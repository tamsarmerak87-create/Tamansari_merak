...
export default function WargaProfilPage() {
    ...
    function hasUnsavedChanges() {
        const currentPhone = profile?.nomor_whatsapp || profile?.nomor_hp || "";
        const currentEmail = profile?.email || user?.email || "";
        return Boolean(file) || telepon.trim() !== currentPhone || email.trim() !== currentEmail || alamat.trim() !== (profile?.alamat || "") || rt.trim() !== (profile?.rt || "") || rw.trim() !== (profile?.rw || "");
    }
    ...
    function goBackSafely() {
        ...
        if (sameOrigin && referrerPath !== "/login" && referrerPath !== "/register" && window.history.length > 1) router.back();
        else router.push("/dashboard");
    }

    function handleBack() {
        if (!hasUnsavedChanges()) return goBackSafely();
        toast.warning("Perubahan belum disimpan", { description: "Data yang Anda ubah belum disimpan." });
    ...
    return <main className="min-h-screen bg-[#F7F9FC] px-4 py-6 text-slate-800 sm:px-8 lg:px-16"><form onSubmit={submitProfile} className="mx-auto max-w-6xl space-y-5"><Header /><div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]"><ProfileCard profile={profile} avatar={avatar} file={file} saving={saving} onPhoto={choosePhoto} /><EditableData saving={saving} telepon={telepon} email={email} alamat={alamat} rt={rt} rw={rw} onTelepon={setTelepon} onEmail={setEmail} onAlamat={setAlamat} onRt={setRt} onRw={setRw} /></div><IdentityData profile={profile} onChangeClick={() => { setChangeOpen(true); toast.info("Data identitas terverifikasi", { description: "Data ini digunakan otomatis untuk pengajuan layanan." }); }} /><div className="grid gap-3 sm:grid-cols-[1fr_auto]"><Button type="submit" variant="gold" disabled={saving} className="min-h-[56px] w-full text-base shadow-[0_14px_35px_rgba(245,179,1,.24)]">{saving ? "⏳ Menyimpan..." : <><Save size={20} /> 💾 Simpan Perubahan</>}</Button><Button type="button" disabled={saving} onClick={handleBack} className="min-h-[56px] w-full border-2 border-emerald-200 bg-white px-6 text-base font-black text-gov-950 hover:bg-emerald-50 sm:w-auto"><ArrowLeft size={20} /> Kembali</Button></div></form>{changeOpen ? <OfficialDataNoticeModal profile={profile} onCancel={() => setChangeOpen(false)} /> : null}{confirmBackOpen ? <UnsavedBackModal onStay={() => setConfirmBackOpen(false)} onLeave={() => { setConfirmBackOpen(false); goBackSafely(); }} /> : null}</main>;
}
...
function UnsavedBackModal({ onStay, onLeave }: { onStay: () => void; onLeave: () => void }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 px-4 py-6 backdrop-blur-sm"><section className="w-full max-w-md rounded-[28px] border border-amber-200 bg-white p-6 shadow-2xl"><div className="rounded-2xl bg-amber-50 p-4"><p className="text-xl font-black text-gov-950">Perubahan belum disimpan</p><p className="mt-2 text-sm font-bold text-slate-600">Data yang Anda ubah belum disimpan.</p></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><Button type="button" onClick={onLeave} className="min-h-[52px] border-2 border-slate-300 bg-white font-black text-slate-700 hover:bg-slate-50">Tetap Kembali</Button><Button type="button" variant="gold" onClick={onStay} className="min-h-[52px] font-black">Tetap di Halaman</Button></div></section></div>; }
