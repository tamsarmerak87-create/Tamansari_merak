...
import { Camera, MapPin, Phone, Save, ShieldCheck, UserRound } from "lucide-react";
...
export default function WargaProfilPage() {
    ...
    useEffect(() => { setTelepon(profile?.nomor_whatsapp || profile?.nomor_hp || ""); }, [profile]);
    ...
    async function submit(event: FormEvent) {
        ...
        try {
            ...
            await updateWargaProfile({ nomor_whatsapp: telepon.trim(), nomor_hp: telepon.trim(), foto_url });
            await refresh();
            setFile(null);
    ...
    return <main className="min-h-screen bg-[#F7F9FC] px-5 py-10 text-slate-800 sm:px-10 lg:px-20"><section className="mx-auto max-w-6xl space-y-6"><Hero title="Profil Saya" text="Data identitas di profil menjadi sumber utama seluruh pengajuan layanan." /><div className="grid gap-6 lg:grid-cols-[.85fr_1.15fr]"><aside className="rounded-[28px] border border-white bg-white p-6 text-center shadow-soft"><div className="mx-auto grid h-36 w-36 place-items-center overflow-hidden rounded-full bg-amber-100 text-amber-600 ring-4 ring-amber-50">{avatar ? <img src={avatar} alt="Foto profil" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} /> : <UserRound size={68} />}</div><h2 className="mt-5 text-2xl font-black text-gov-950">{profile.nama_lengkap}</h2><p className="mt-1 text-sm font-bold text-slate-500">NIK: {mask(profile.nik)}</p><p className="mt-3 inline-flex rounded-full bg-emerald-100 px-3 py-1 text-sm font-black text-emerald-700">Data Profil Terverifikasi</p></aside><form onSubmit={submit} className="rounded-[28px] border border-white bg-white p-6 shadow-soft"><div className="grid gap-4 sm:grid-cols-2"><Field label="Nama Lengkap"><input value={profile.nama_lengkap || "-"} readOnly className="input bg-slate-50 text-slate-500" /></Field><Field label="NIK"><input value={mask(profile.nik)} readOnly className="input bg-slate-50 text-slate-500" /></Field><Field label="Alamat"><input value={profile.alamat || "-"} readOnly className="input bg-slate-50 text-slate-500" /></Field><Field label="Kelurahan / Kecamatan / Kota"><input value={`${profile.kelurahan || "Tamansari"}, ${profile.kecamatan || "Pulomerak"}, Kota Cilegon`} readOnly className="input bg-slate-50 text-slate-500" /></Field><Field label="Nomor WhatsApp / Telepon"><input value={telepon} onChange={(e) => setTelepon(e.target.value)} className="input" placeholder="Nomor WhatsApp / Telepon" /></Field><Field label="Email (read-only)"><input value={profile.email || user.email || ""} readOnly className="input bg-slate-50 text-slate-500" /></Field><div className="sm:col-span-2"><label className="mb-2 block text-sm font-black text-gov-950">Ganti Foto Profil</label><label className="flex cursor-pointer items-center justify-center gap-3 rounded-2xl border border-dashed border-amber-300 bg-amber-50 px-4 py-5 text-sm font-black text-amber-700"><Camera size={18} /> {file ? `File dimuat: ${file.name}` : "Pilih gambar JPG/PNG/WebP maksimal 5 MB"}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={choosePhoto} className="hidden" disabled={saving} /></label>{uploading ? <p className="mt-2 text-sm font-bold text-amber-700">Mengunggah foto...</p> : null}</div></div><p className="mt-4 rounded-2xl bg-amber-50 p-4 text-sm font-bold text-amber-800">Nama, NIK, dan alamat terkunci agar tetap sama dengan data terverifikasi petugas.</p>{error ? <p className="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</p> : null}{message ? <p className="mt-4 rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700">{message}</p> : null}<div className="mt-6 flex flex-wrap gap-3"><Button type="submit" variant="gold" disabled={saving}>{saving ? (uploading ? "Mengunggah foto..." : "Menyimpan perubahan...") : <><Save size={18} /> Simpan Perubahan</>}</Button><Button type="button" variant="glass" href="/dashboard">Kembali ke Dashboard</Button></div></form></div><InfoGrid profile={profile} email={user.email || profile.email} /></section></main>;
}
