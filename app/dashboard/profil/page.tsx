...
import { getWargaProfilePhotoUrl, submitWargaProfileChangeRequest, updateWargaProfile, uploadWargaProfileChangeDocument, uploadWargaProfilePhoto, WARGA_RELIGIONS, type WargaProfile } from "@/services/warga-auth.service";
import { compressWargaFile, MAX_WARGA_FILE_SIZE } from "@/services/warga-file-compress";

const maxPhoto = MAX_WARGA_FILE_SIZE;
...
export default function WargaProfilPage() {
    ...
    async function choosePhoto(event: ChangeEvent<HTMLInputElement>) {
        ...
        try { selected = await compressWargaFile(selected); } catch (error) { return void toast.warning("Foto tidak dapat diproses", { description: error instanceof Error ? error.message : "Silakan coba foto lain." }); }
        if (selected.size > maxPhoto) return void toast.warning("Ukuran foto terlalu besar", { description: "Maksimal 1 MB." });
        setFile(selected);
    ...
    async function submitProfile(event: FormEvent) {
        ...
        const changed = file || telepon.trim() !== currentPhone || email.trim() !== currentEmail || alamat.trim() !== (profile?.alamat || "") || rt.trim() !== (profile?.rt || "") || rw.trim() !== (profile?.rw || "") || agama !== (profile?.agama || "");
        ...
        if (!WARGA_RELIGIONS.includes(agama as (typeof WARGA_RELIGIONS)[number])) return void toast.warning("Agama wajib dipilih", { description: "Pilih agama sesuai data profil Anda." });
        ...
        try {
            ...
            await updateWargaProfile({ nomor_whatsapp: telepon.trim(), nomor_hp: telepon.trim(), email: email.trim(), alamat: alamat.trim(), rt: rt.trim(), rw: rw.trim(), agama, foto_url });
            await refresh();
            setFile(null);
            toast.update(toastId, "success", "✓ Profil berhasil diperbarui.");
    ...
    return <main className="min-h-screen bg-[#F7F9FC] px-4 py-6 text-slate-800 sm:px-8 lg:px-16"><form onSubmit={submitProfile} className="mx-auto max-w-6xl space-y-5"><Header onBack={() => router.push("/dashboard")} /><div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]"><ProfileCard profile={profile} avatar={avatar} file={file} saving={saving} onPhoto={choosePhoto} /><EditableData saving={saving} telepon={telepon} email={email} alamat={alamat} rt={rt} rw={rw} agama={agama} onTelepon={setTelepon} onEmail={setEmail} onAlamat={setAlamat} onRt={setRt} onRw={setRw} onAgama={setAgama} /></div><IdentityData profile={profile} onChangeClick={() => { setChangeOpen(true); toast.info("Data identitas terverifikasi", { description: "Data ini digunakan otomatis untuk pengajuan layanan." }); }} /><Button type="submit" variant="gold" disabled={saving} className="min-h-[58px] w-full text-base uppercase shadow-[0_14px_35px_rgba(245,179,1,.24)]">{saving ? "⏳ Menyimpan..." : <><Save size={20} /> 💾 Simpan Perubahan</>}</Button></form>{changeOpen ? <OfficialDataNoticeModal profile={profile} onCancel={() => setChangeOpen(false)} /> : null}</main>;
}

function Header({ onBack }: { onBack: () => void }) { return <section className="rounded-[28px] bg-white p-6 shadow-soft ring-1 ring-slate-100"><div className="mb-5 flex"><button type="button" onClick={onBack} className="inline-flex min-h-[44px] w-full items-center justify-center rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-black text-slate-800 shadow-sm transition hover:border-slate-400 hover:bg-slate-50 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-slate-200 sm:w-auto">← Kembali ke Dashboard</button></div><div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-black uppercase tracking-tight text-gov-950 sm:text-4xl">PROFIL WARGA</h1><p className="mt-2 max-w-2xl text-sm font-bold text-slate-600 sm:text-base">Kelola data profil Anda. Data ini digunakan otomatis untuk pengajuan layanan.</p></div><p className="rounded-full bg-emerald-100 px-4 py-2 text-sm font-black text-emerald-700">✓ Profil Terverifikasi</p></div></section>; }
function ProfileCard({ profile, avatar, file, saving, onPhoto }: { profile: WargaProfile; avatar: string | null; file: File | null; saving: boolean; onPhoto: (e: ChangeEvent<HTMLInputElement>) => void }) { return <section className="rounded-[28px] border border-white bg-white p-6 text-center shadow-soft"><div className="mx-auto grid h-36 w-36 place-items-center overflow-hidden rounded-full bg-amber-100 text-amber-600 ring-4 ring-amber-50">{avatar ? <img src={avatar} alt="Foto profil" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} /> : <UserRound size={68} />}</div><h2 className="mt-5 text-2xl font-black text-gov-950">{profile.nama_lengkap}</h2><p className="mt-1 text-sm font-bold text-slate-500">NIK: {mask(profile.nik)}</p><p className="mt-3 inline-flex rounded-full bg-emerald-100 px-3 py-1 text-sm font-black text-emerald-700">✓ Data Profil Terverifikasi</p><label className="mx-auto mt-5 flex min-h-[48px] w-full cursor-pointer items-center justify-center gap-2 rounded-2xl bg-accent-400 px-5 py-3 text-sm font-black text-gov-950 shadow-soft hover:bg-accent-200 sm:w-auto"><Camera size={18} /> Ubah Foto<input type="file" accept="image/jpeg,image/png,image/webp" onChange={onPhoto} className="hidden" disabled={saving} /></label><p className="mt-3 text-xs font-bold text-slate-500">JPG / PNG / WebP, maksimal 1 MB. {file ? `Preview siap: ${file.name}` : "Preview tampil sebelum disimpan."}</p></section>; }
function EditableData(props: { saving: boolean; telepon: string; email: string; alamat: string; rt: string; rw: string; agama: string; onTelepon: (v: string) => void; onEmail: (v: string) => void; onAlamat: (v: string) => void; onRt: (v: string) => void; onRw: (v: string) => void; onAgama: (v: string) => void }) { return <section className="rounded-[28px] border-2 border-amber-200 bg-amber-50/65 p-5 shadow-soft sm:p-6"><p className="text-xs font-black uppercase tracking-[.18em] text-amber-700">DATA YANG BISA DIUBAH</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Nomor WhatsApp / Telepon"><Input value={props.telepon} onChange={props.onTelepon} disabled={props.saving} inputMode="tel" placeholder="0812234567890" /></Field><Field label="Email"><Input type="email" value={props.email} onChange={props.onEmail} disabled={props.saving} placeholder="ratih@gmail.com" /></Field><Field label="Agama"><select aria-label="Agama" required value={props.agama} onChange={(event) => props.onAgama(event.target.value)} disabled={props.saving} className="input min-h-[54px] border-2 border-slate-300 bg-white px-4 text-base font-bold shadow-sm focus:border-amber-400 focus:ring-4 focus:ring-amber-100"><option value="">Pilih agama</option>{WARGA_RELIGIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></Field><Field label="Alamat Domisili"><Input value={props.alamat} onChange={props.onAlamat} disabled={props.saving} placeholder="langon indah" /></Field><Field label="RT"><Input value={props.rt} onChange={props.onRt} disabled={props.saving} inputMode="numeric" placeholder="05" /></Field><Field label="RW"><Input value={props.rw} onChange={props.onRw} disabled={props.saving} inputMode="numeric" placeholder="12" /></Field></div></section>; }
...
function identityRows(profile: WargaProfile) { return [["Nama Lengkap", profile.nama_lengkap || "-"], ["NIK", mask(profile.nik)], ["Tempat/Tanggal Lahir", `${profile.tempat_lahir || "-"}${profile.tanggal_lahir ? `, ${profile.tanggal_lahir}` : ""}`], ["Jenis Kelamin", profile.jenis_kelamin || "-"], ["Agama", profile.agama || "-"], ["Nomor KK", mask(profile.nomor_kk)], ["Kelurahan", profile.kelurahan || "Tamansari"], ["Kecamatan", profile.kecamatan || "Pulomerak"], ["Kota", "Cilegon"], ["Status Perkawinan", profile.status_perkawinan || "-"], ["Status Pekerjaan", profile.status_pekerjaan || "-"]]; }
...
function officialFieldOptions(profile: WargaProfile) { return [{ key: "nama_lengkap", label: "Nama Lengkap", value: profile.nama_lengkap }, { key: "nik", label: "NIK", value: mask(profile.nik) }, { key: "tempat_tanggal_lahir", label: "Tempat/Tanggal Lahir", value: `${profile.tempat_lahir || "-"}, ${profile.tanggal_lahir || "-"}` }, { key: "jenis_kelamin", label: "Jenis Kelamin", value: profile.jenis_kelamin || "-" }, { key: "nomor_kk", label: "Nomor KK", value: mask(profile.nomor_kk) }, { key: "kelurahan", label: "Kelurahan", value: profile.kelurahan || "Tamansari" }, { key: "kecamatan", label: "Kecamatan", value: profile.kecamatan || "Pulomerak" }, { key: "kota", label: "Kota", value: "Cilegon" }]; }