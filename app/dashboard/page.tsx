...
function BottomNav() {
    ...
    return <nav aria-label="Navigasi dashboard warga" className="fixed left-1/2 bottom-[max(12px,env(safe-area-inset-bottom))] z-[45] flex h-[72px] w-[calc(100vw-24px)] max-w-[460px] -translate-x-1/2 items-center justify-around rounded-[24px] border border-[#E8E8E8] bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] shadow-[0_18px_50px_rgba(15,23,42,.18)] backdrop-blur-xl transition-all duration-200 ease-out sm:h-[76px] sm:w-[calc(100vw-32px)] md:max-w-[480px] lg:h-[78px]">{items.map(([label, href, Icon, active], i) => { const isScan = i === 2; return <a key={label} href={href} aria-current={active ? "page" : undefined} className={`group flex min-h-12 min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-2xl px-1 text-center text-[10px] font-black leading-tight transition duration-200 active:scale-95 sm:text-[11px] ${active ? "text-[#F0A000]" : "text-slate-600 hover:text-[#15803D]"} ${isScan ? "-mt-7" : "hover:bg-emerald-50/80"}`}><span className={`grid place-items-center transition duration-200 ${isScan ? "h-[58px] w-[58px] rounded-full bg-[#16A34A] text-white shadow-[0_12px_28px_rgba(22,163,74,.32)] ring-4 ring-white group-hover:bg-[#15803D] sm:h-16 sm:w-16" : active ? "h-7 w-7 rounded-xl bg-[#FFF8DB]" : "h-7 w-7"}`}><Icon size={isScan ? 27 : 21} /></span><span className="max-w-full truncate">{label}</span></a>; })}</nav>;
}
