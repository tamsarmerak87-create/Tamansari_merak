...
import { AlertCircle, Barcode, Bell, ChevronDown, ChevronRight, ClipboardList, Clock3, FileCheck2, FileText, FolderOpen, Headphones, HelpCircle, Home, Images, LogOut, MapPin, Menu, MessageSquareText, Phone, QrCode, RefreshCw, Scale, Search, Settings, ShieldCheck, UserRound, X, type LucideIcon } from "lucide-react";
...
const quick: ReadonlyArray<readonly [string, string, LucideIcon, string]> = [["Ajukan Surat", "Buat pengajuan baru", FileText, "layanan"], ["Cek Status", "Lihat proses pengajuan", RefreshCw, "pengajuan"], ["Tracking Dokumen", "Lacak pengajuan Anda", MapPin, "tracking"], ["Dokumen Saya", "Lihat dokumen tersimpan", FolderOpen, "dokumen"], ["Pengaduan", "Sampaikan keluhan", MessageSquareText, "/pengaduan"]];
const otherServices: ReadonlyArray<readonly [string, string, LucideIcon]> = [["POSBANKUM", "/posbankum", Scale], ["FAQ", "/faq", HelpCircle], ["Kontak", "/kontak", Phone], ["Galeri", "/galeri", Images]];
...
export default function DashboardPage() {
    ...
    return <main className="min-h-screen bg-[#F7F8F5] text-[#172033]">
        ...
        <div className="flex"><Sidebar open={sidebar} setOpen={setSidebar} active={active} onGo={go} /><section className="min-w-0 flex-1 px-4 pb-[calc(var(--mobile-bottom-nav-height)+env(safe-area-inset-bottom)+20px)] pt-5 sm:px-6 lg:px-8 lg:pb-8"><div className="mx-auto max-w-[1400px] space-y-6"><Welcome profile={profile} stats={stats} /><section id="home"><SectionTitle title="Akses Cepat" description="Pilih kebutuhan utama Anda" href="/layanan" /><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">{quick.map(([title, desc, Icon, target], i) => <button key={title} onClick={() => void go(target)} className="group flex min-h-36 flex-col items-start rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-[0_4px_18px_rgba(15,23,42,.04)] transition hover:-translate-y-1 hover:border-[#e7b800] hover:shadow-lg sm:min-h-0 sm:flex-row sm:items-center sm:gap-4 sm:p-5"><span className={`mb-3 grid h-12 w-12 shrink-0 place-items-center rounded-2xl sm:mb-0 ${i % 2 ? "bg-emerald-50 text-[#15803D]" : "bg-amber-50 text-[#D89000]"}`}><Icon size={27} /></span><span className="min-w-0"><b className="text-sm sm:text-base">{title}</b><p className="mt-1 text-xs leading-5 text-slate-500 sm:text-sm">{desc}</p></span></button>)}</div></section><div className="grid gap-5 xl:grid-cols-[1.08fr_.92fr]"><Recent items={items} fetching={fetching} error={error} /><Tracking item={latest} fetching={fetching} error={error} query={query} setQuery={setQuery} search={searchTracking} refresh={() => void refresh()} message={message} /></div>{latest && <section className="grid gap-5 lg:grid-cols-[1fr_320px]"><div className="rounded-2xl border border-slate-200 bg-[linear-gradient(135deg,#fff,#fff9df_55%,#eef9f1)] p-5 shadow-sm"><p className="font-black">Barcode Pengajuan</p><p className="mt-1 text-sm font-semibold text-slate-500">Tunjukkan kode ini kepada petugas saat datang ke Kelurahan.</p><div className="mt-4 overflow-hidden text-ellipsis rounded-xl bg-white p-4 font-mono text-lg font-black shadow-sm">{agenda(latest)}</div></div><QRCodePelayanan nomorPengajuan={agenda(latest)} status={latest.status} tanggal={date(latest.created_at)} layanan={docName(latest)} size={188} /></section>}<Documents docs={docs} fetching={fetching} /><Info /><OtherServices /><FooterDash /></div></section></div><BottomNav />
    </main>;
}
...
function OtherServices() { return <section><SectionTitle title="Layanan Lainnya" description="Akses informasi dan layanan kelurahan lainnya" /><div className="grid grid-cols-2 gap-3 md:grid-cols-4">{otherServices.map(([label, href, Icon]) => <a key={label} href={href} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 font-black shadow-sm transition hover:border-emerald-300 hover:bg-emerald-50"><span className="grid h-10 w-10 place-items-center rounded-xl bg-amber-50 text-[#D89000]"><Icon size={21} /></span>{label}</a>)}</div></section>; }
