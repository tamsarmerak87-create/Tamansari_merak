...
function Welcome({ profile, stats }: { profile: WargaProfile; stats: { waiting: number; process: number; done: number } }) { const summary: Array<[number, string, LucideIcon]> = [[stats.waiting, "Menunggu", Clock3], [stats.process, "Diproses", RefreshCw], [stats.done, "Selesai", FileCheck2]]; return <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_30px_rgba(15,23,42,.06)]"><h1 className="text-2xl font-extrabold">Selamat datang, {profile.nama_lengkap}! 👋</h1><p className="mt-1 text-slate-500">Kelurahan Tamansari, Kecamatan Pulomerak, Kota Cilegon</p><div className="mt-5 grid gap-5 rounded-2xl bg-gradient-to-br from-[#1557D6] to-[#0B3EA6] p-5 text-white lg:grid-cols-[1fr_1.1fr]"><div className="flex flex-wrap items-center gap-5"><div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-white/20 ring-4 ring-white/15">{profile.foto_url ? <Image src={profile.foto_url} alt={profile.nama_lengkap} width={80} height={80} className="h-full w-full object-cover" unoptimized /> : <UserRound size={42} />}</div><div><div className="flex items-center gap-2"><p className="text-xl font-extrabold">{profile.nama_lengkap}</p><span className="rounded-full bg-white/20 px-3 py-1 text-xs font-bold">Warga</span></div><p className="mt-2 text-sm text-blue-50">NIK: {mask(profile.nik)}</p><p className="text-sm text-blue-50">No. WhatsApp: {mask(profile.nomor_whatsapp)}</p><a href="/dashboard/profil" className="mt-4 inline-flex rounded-xl bg-white px-4 py-2 text-sm font-bold text-[#1557D6]">Lihat Profil</a></div></div><div className="rounded-2xl bg-white p-5 text-[#14213d]"><p className="font-extrabold">Ringkasan Pengajuan</p><div className="mt-5 grid grid-cols-3 divide-x">{summary.map(([n, label, Icon]) => <div key={label} className="px-3"><Icon className="mb-2 text-[#1557D6]" /><p className="text-3xl font-extrabold">{n}</p><p className="text-sm font-bold">{label}</p></div>)}</div></div></div></motion.section>; }
        ...
        <div className="relative">
            <button onClick={() => setOpen(!open)} className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-amber-100 text-amber-600">
                    {profile.foto_url ? <Image src={profile.foto_url} alt={profile.nama_lengkap} width={40} height={40} className="h-full w-full object-cover" unoptimized /> : <UserRound />}
                </div>
