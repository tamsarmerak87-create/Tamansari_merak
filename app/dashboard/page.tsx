...
function BottomNav() {
    ...
    return <nav aria-label="Navigasi dashboard warga" className="fixed inset-x-0 bottom-0 z-50 flex min-h-[var(--mobile-bottom-nav-height)] items-center justify-around rounded-t-[22px] border-t border-[#E8E8E8] bg-white/95 px-2 pb-[max(env(safe-area-inset-bottom),8px)] pt-2 shadow-[0_-10px_30px_rgba(15,23,42,.12)] backdrop-blur-xl transition-all duration-200 ease-out lg:hidden">{items.map(([label, href, Icon, active], i) => { const isScan = i === 2; return <a key={label} href={href} aria-current={active ? "page" : undefined} className={`group flex min-h-12 min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-2xl px-1 text-center text-[10px] font-black leading-tight transition duration-200 active:scale-95 sm:text-[11px] ${active ? "text-[#F0A000]" : "text-slate-600 hover:text-[#15803D]"} ${isScan ? "-mt-3" : "hover:bg-emerald-50/80"}`}><span className={`grid place-items-center transition duration-200 ${isScan ? "h-[52px] w-[52px] rounded-full bg-[#16A34A] text-white shadow-[0_8px_20px_rgba(22,163,74,.28)] ring-4 ring-white group-hover:bg-[#15803D] sm:h-14 sm:w-14" : active ? "h-7 w-7 rounded-xl bg-[#FFF8DB]" : "h-7 w-7"}`}><Icon size={isScan ? 25 : 21} /></span><span className="max-w-full truncate">{label}</span></a>; })}</nav>;
}
