...
import { getMyDocumentsFromPengajuan, getMyPengajuan, type DokumenPengajuan } from "@/services/warga-pengajuan.service";
...
export default function DokumenSayaPage() {
    ...
    const grouped = useMemo(() => docs.filter((doc) => Boolean(doc.url_file)), [docs]);

    if (loading || !user) return <main className="min-h-screen bg-[#F7F9FC] p-10 font-black text-gov-950">Memuat dokumen...</main>;

    return <main className="min-h-screen bg-[#F7F9FC] px-5 py-10 text-slate-800 sm:px-10 lg:px-20"><section className="mx-auto max-w-6xl space-y-6"><Hero title="Dokumen Saya" text="Semua dokumen ditarik dari data pengajuan milik akun yang sedang login." /><div className="flex justify-end"><Button type="button" variant="glass" onClick={() => void refresh()}><RefreshCw size={18} /> Refresh</Button></div>{fetching ? <State text="Memuat dokumen..." /> : error ? <State text={error} retry onRetry={refresh} /> : grouped.length === 0 ? <Empty /> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{grouped.map((doc) => <article key={doc.id ?? `${doc.pengajuan_id}-${doc.url_file}`} className="rounded-[26px] border border-white bg-white p-5 shadow-soft"><div className="flex items-start justify-between gap-3"><div><FileText className="text-accent-400" /><p className="mt-3 text-lg font-black text-gov-950">{doc.nama_file || doc.jenis || "Dokumen"}</p><p className="mt-1 text-sm font-bold text-slate-500">No. Agenda: {doc.nomor_pengajuan || "-"}</p><p className="mt-1 text-xs font-bold text-slate-400">Status: {doc.status || "Tersedia"}</p></div><span className="rounded-full bg-gov-50 px-3 py-1 text-xs font-black text-gov-900 ring-1 ring-slate-200">{doc.jenis || "File"}</span></div><div className="mt-5 flex flex-wrap gap-2"><a href={doc.url_file || "#"} target="_blank" rel="noreferrer" className="inline-flex min-h-[44px] items-center gap-2 rounded-2xl border border-border-soft bg-white px-4 py-3 text-sm font-black text-gov-900 shadow-soft"><Eye size={16} /> Lihat</a><a href={doc.url_file || "#"} download className="inline-flex min-h-[44px] items-center gap-2 rounded-2xl bg-accent-400 px-4 py-3 text-sm font-black text-gov-950 shadow-soft"><Download size={16} /> Download</a></div></article>)}</div>}</section></main>;
}
