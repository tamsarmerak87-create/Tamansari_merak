...
interface WargaVerificationDetail extends Row {
    handled_by?: string | null;
    handled_by_name: string | null;
    handled_by_nip: string | null;
    handled_by_jabatan: string | null;
...
const knownFields = new Set(["nama_lengkap", "nik", "nomor_kk", "tempat_lahir", "tanggal_lahir", "jenis_kelamin", "email", "nomor_hp", "nomor_whatsapp", "alamat", "rt", "rw", "kelurahan", "kecamatan", "status_verifikasi", "tahap_verifikasi", "created_at", "handled_by", "handled_by_name", "handled_by_nip", "handled_by_jabatan"]);
...
export function PetugasWargaVerification({ id }: { id: string }) {
    ...
    const [row, setRow] = useState<WargaVerificationDetail | null>(null);
    ...
    return <main className="min-h-screen overflow-x-hidden bg-slate-50 p-4 text-slate-900 md:p-8">
        <div className="mx-auto max-w-6xl space-y-5">
            ...
            <header className="rounded-[2rem] bg-gov-950 p-5 text-white md:p-6">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-4">{[["Status", row.status_verifikasi], ["Tahap", current], ["Petugas menangani", row.handled_by_name || "Belum ditugaskan"], ["Role/tahap petugas", row.active_stage?.label ?? profile.role]].map(([a, b]) => <div key={String(a)} className="rounded-2xl bg-white/10 p-3"><p className="text-xs font-black uppercase opacity-80">{a}</p><p className="break-words font-black">{showValue(b)}</p>{a === "Petugas menangani" && row.handled_by_name && <p className="mt-1 text-xs opacity-80">{[row.handled_by_nip && `NIP ${row.handled_by_nip}`, row.handled_by_jabatan].filter(Boolean).join(" · ")}</p>}</div>)}</div>
            </header>
            <Section title="Profil Warga"><div className="grid gap-4 md:grid-cols-2"><InfoCard label="Nama lengkap" value={row.nama_lengkap} /><InfoCard label="NIK" value={row.nik} /><InfoCard label="Nomor KK" value={row.nomor_kk} /><InfoCard label="Tempat lahir" value={row.tempat_lahir} /><InfoCard label="Tanggal lahir" value={row.tanggal_lahir} /><InfoCard label="Jenis kelamin" value={row.jenis_kelamin} /></div><div className="mt-4 grid gap-4 md:grid-cols-3"><InfoCard label="Email" value={row.email} /><InfoCard label="Nomor HP" value={row.nomor_hp} /><InfoCard label="Nomor WhatsApp" value={row.nomor_whatsapp} /></div><div className="mt-4 grid gap-4 md:grid-cols-5"><div className="md:col-span-2"><InfoCard label="Alamat lengkap" value={row.alamat} /></div><InfoCard label="RT" value={row.rt} /><InfoCard label="RW" value={row.rw} /><InfoCard label="Kelurahan" value={row.kelurahan} /><InfoCard label="Kecamatan" value={row.kecamatan} /></div><div className="mt-4 grid gap-4 md:grid-cols-4"><InfoCard label="Status verifikasi" value={row.status_verifikasi} /><InfoCard label="Tahap verifikasi saat ini" value={current} /><InfoCard label="Tanggal pendaftaran" value={formatDate(row.created_at)} /><InfoCard label="Petugas yang menangani" value={row.handled_by_name || "Belum ditugaskan"} /></div>{extraFields.length > 0 && <div className="mt-4 grid gap-4 md:grid-cols-3">{extraFields.map(([key, value]) => <InfoCard key={key} label={pretty(key)} value={value} />)}</div>}</Section>
