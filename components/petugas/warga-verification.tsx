...
function isImage(doc: Row) { return /gambar|jpg|jpeg|png|webp/i.test(`${doc.tipe_file} ${doc.nama_file}`); }
...
export function PetugasWargaVerification({ id }: { id: string }) {
    ...
    return <main className="min-h-screen overflow-x-hidden bg-slate-50 p-4 text-slate-900 md:p-8">
        <div className="mx-auto max-w-6xl space-y-5">
            ...
            <Section title="Dokumen Yang Diupload">{documents.length ? <div className="grid gap-4 lg:grid-cols-2">{documents.map((doc: Row) => <article key={doc.id} className="rounded-3xl border bg-slate-50 p-4"><div className="flex flex-wrap justify-between gap-3"><div><p className="text-xl font-black text-gov-950">{doc.nama_dokumen}</p><p className="text-sm font-bold text-slate-600">{doc.jenis_dokumen} - {doc.nama_file}</p></div><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-800">{doc.status ?? "Dokumen tersedia"}</span></div><div className="mt-4 overflow-hidden rounded-2xl border bg-white">{doc.file_url && isImage(doc) ? <img src={doc.file_url} alt={doc.nama_dokumen} className="max-h-80 w-full object-contain" /> : doc.file_url && isPdf(doc) ? <iframe src={doc.file_url} title={doc.nama_dokumen} className="h-80 w-full" /> : <div className="grid h-40 place-items-center p-4 text-center font-bold text-slate-500">Preview tidak tersedia untuk tipe file ini.</div>}</div><div className="mt-4 grid gap-2 text-sm font-bold text-slate-600 sm:grid-cols-3"><p>Jenis: {doc.tipe_file ?? "-"}</p><p>Ukuran: {formatBytes(doc.ukuran_file)}</p><p>Upload: {formatDate(doc.uploaded_at)}</p></div><p className="mt-2 text-xs font-bold text-slate-500">Tautan dokumen dibuat sebagai URL aman, bukan path storage mentah.</p>{doc.file_url ? <a href={doc.file_url} target="_blank" rel="noreferrer" className="mt-4 inline-flex w-full justify-center rounded-2xl bg-gov-950 px-4 py-3 font-black text-white sm:w-auto">Lihat Dokumen</a> : <p className="mt-4 rounded-2xl bg-red-50 p-3 font-bold text-red-700">File tidak dapat dibuatkan tautan aman.</p>}</article>)}</div> : <p className="rounded-2xl bg-slate-100 p-4 font-bold text-slate-600">Belum ada dokumen yang diupload warga.</p>}</Section>
