...
export function QRCodePelayanan({ nomorPengajuan, size = 180, showActions = true, status, tanggal, layanan }: QRCodePelayananProps) {
    ...
    return <section className="rounded-[24px] border border-[#E8E8E8] bg-white p-5 text-center shadow-sm" aria-label={`QR Code pelayanan ${number}`}>
        <div className="mb-4 text-left"><div className="flex items-center gap-2 text-sm font-black text-[#172033]"><QrCode className="text-[#16A34A]" size={18} />QR Code Pelayanan</div><p className="mt-1 text-xs font-semibold text-slate-500">Tunjukkan QR Code ini kepada petugas saat datang ke Kelurahan.</p></div>
        ...
        {showActions && <div className="mt-4 flex flex-wrap justify-center gap-2"><button type="button" onClick={downloadQr} aria-label="Unduh QR Code pelayanan" className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#16A34A]/50 bg-white px-4 text-sm font-black text-[#15803D] transition hover:bg-emerald-50 focus:outline-none focus:ring-4 focus:ring-[#16A34A]/20"><Download size={16} />Unduh QR</button><button type="button" onClick={printQr} aria-label="Cetak QR Code pelayanan" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#FFC400] px-4 text-sm font-black text-[#172033] transition hover:bg-[#FFD84D] focus:outline-none focus:ring-4 focus:ring-[#FFC400]/40"><Printer size={16} />Cetak QR</button></div>}
    </section>;
}