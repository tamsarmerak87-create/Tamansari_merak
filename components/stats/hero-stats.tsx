            ...
            {stats.map((item, index) => {
                ...
                return (
                    <MotionShell key={item.label} delay={index * 0.04}>
                        <div className="group flex min-h-[6.25rem] items-center gap-4 rounded-[1.7rem] border border-white/85 bg-white/82 p-4 shadow-soft backdrop-blur-xl transition hover:-translate-y-1 hover:scale-[1.01] hover:bg-white hover:shadow-gold">
                            ...
                            <div>
                                <b className="block text-base font-black leading-tight text-gov-950 sm:text-lg">{item.label}</b>
                                <span className="mt-1 block text-sm font-semibold leading-5 text-slate-650">{item.desc}</span>
                            </div>
                        </div>
                    </MotionShell>
                );
